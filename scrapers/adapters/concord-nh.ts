...
/**
 * Example adapter for Concord, NH.
 * Each municipality can have its own adapter that handles
 * site-specific HTML structure and quirks.
 */
export class ConcordNhScraper implements ScraperAdapter {
  ...
  async scrape(config: ScraperConfig): Promise<ScrapedRfp[]> {
    ...
    try {
      ...
      // Adapt selectors to the actual page structure
      // This is a template — adjust once the real page is inspected
      $("table.bids-table tbody tr, .rfp-listing .rfp-item").each((_, el) => {
        ...
        rfps.push({
          ...
          deadline_date: this.parseDate(deadlineText),
          posted_date: this.parseDate(postedText),
  ...
  private parseDate(str: string): string | undefined {
